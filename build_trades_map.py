...
function saveCSV() {
  ...
  const cols = ['Business Name','Street Address','Suburb','Phone','Website','Google Rating','Trade Category'];
  const rows = [cols.join(',')];
  allPlaces.forEach(p => rows.push([
    csv(p.name), csv(p.street), csv(p.suburb),
    csv(p.phone), csv(p.website), p.rating || '', csv(p.category),
  ].join(',')));
