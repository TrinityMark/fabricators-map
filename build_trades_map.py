...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #abn-modal  { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 10000; align-items: center; justify-content: center; }
    #abn-modal.active  { display: flex; }
    #here-modal { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 10000; align-items: center; justify-content: center; }
    #here-modal.active { display: flex; }
...
<div id="abn-modal">
  <div class="key-box">
    <h2>ABN Lookup GUID</h2>
    <p>Enter your ABN Lookup web services GUID. Used to find businesses by street address and enrich owner names from the Australian Business Register. Register free at <a href="https://abr.business.gov.au/Tools/WebServices" target="_blank">abr.business.gov.au</a>.</p>
    <input type="password" id="abn-input" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" />
    <div class="row">
      <button class="btn" style="background:#ddd;color:#333;" onclick="closeAbnModal()">Cancel</button>
      <button class="btn btn-go" onclick="saveAbnGuid()">Save</button>
    </div>
  </div>
</div>
...
<div id="here-modal">
  <div class="key-box">
    <h2>HERE Places API Key</h2>
    <p>Enter your HERE API key. Used to find every business near a street regardless of type — catches businesses that Google Maps search misses. Get a free key (250k requests/month) at <a href="https://developer.here.com" target="_blank">developer.here.com</a>.</p>
    <input type="password" id="here-input" placeholder="your-here-api-key" />
    <div class="row">
      <button class="btn" style="background:#ddd;color:#333;" onclick="closeHereModal()">Cancel</button>
      <button class="btn btn-go" onclick="saveHereKey()">Save</button>
    </div>
  </div>
</div>
...
<button class="btn btn-csv" id="btn-csv" onclick="saveCSV()" disabled>&#11123; Save CSV</button>
    ...
    <button class="btn btn-key" onclick="promptHereKey()" id="btn-here" title="HERE Places API Key">&#128205; HERE</button>
...
async function runSearch() {
  ...
  try {
    ...
    let places;
    let streetCenter = center; // may be updated to precise street coords below
    const isStreetSearch = !!street;

    if (street) {
      // ══ Street mode — three passes to maximise coverage ════════════════
      //
      // Pass 1 — coordinates map scan (primary)
      //   Build a Google Maps search URL from the street's geocoded location.
      //   This tells Google "show me every business pinned on the map here"
      //   regardless of business type — catches engineers, kayak shops, etc.
      //
      // Pass 2 — expanded text queries (backup)
      //   24 trade terms + 20 broad terms × 20 results each.
      //   Catches businesses that don't show in the map view zoom.
      //
      // Pass 3 — plain street name query (final catch-all)
      //   Sometimes returns listings the other passes miss.
      //
      // All three are merged, deduplicated, then filtered to this street.

      // Try to geocode the street precisely; fall back to suburb centre
      streetCenter = center;
      try {
        setStatus(`Locating ${street}…`);
        streetCenter = await geocode(`${street} ${suburb} Australia`);
      } catch (e) { /* use suburb centre */ }
      map.setView([streetCenter.lat, streetCenter.lng], 17);

      // ── Pass 1: coordinates URL ─────────────────────────────
      setStatus(`Pass 1 — scanning map at ${street}…`);
      const mapsUrl = `https://www.google.com/maps/search/businesses/@${streetCenter.lat.toFixed(6)},${streetCenter.lng.toFixed(6)},17z`;
      const runId1     = await apifyRun({ startUrls: [{ url: mapsUrl }], maxCrawledPlacesPerSearch: 200, language: 'en', maxImages: 0, scrapeReviews: false }, token);
      const datasetId1 = await apifyWait(runId1, token);
      let raw = await apifyDataset(datasetId1, token);
      setStatus(`Pass 1 done (${raw.length} raw) — running text queries…`);

      // ── Pass 2: expanded text queries ──────────────────────
      const broadTerms = [
        'business','company','service','contractor','supplier','warehouse',
        'office','shop','store','workshop','engineer','manufacturing',
        'equipment','rental','specialist','consultant','distributor',
        'supplies','products','industries',
      ];
      const textQueries = [
        ...TRADES.map(([, q]) => `${q} ${street} ${suburb} Australia`),
        ...broadTerms.map(t  => `${t} ${street} ${suburb} Australia`),
      ];
      const runId2     = await apifyRun({ searchStringsArray: textQueries, maxCrawledPlacesPerSearch: 20, language: 'en', maxImages: 0, scrapeReviews: false }, token);
      const datasetId2 = await apifyWait(runId2, token);
      const raw2 = await apifyDataset(datasetId2, token);
      raw = raw.concat(raw2);
      setStatus(`Pass 2 done (${raw.length} combined) — running catch-all…`);

      // ── Pass 3: plain street name ───────────────────────────
      const runId3     = await apifyRun({ searchStringsArray: [`${street} ${suburb} Australia`], maxCrawledPlacesPerSearch: 100, language: 'en', maxImages: 0, scrapeReviews: false }, token);
      const datasetId3 = await apifyWait(runId3, token);
      const raw3 = await apifyDataset(datasetId3, token);
      raw = raw.concat(raw3);
      setStatus(`All passes done (${raw.length} total raw) — filtering…`);

      // ── Build places from merged raw results ────────────────
      const seen = new Set();
      places = [];
      for (const p of raw) {
        ...
        const parsed   = p.addressParsed || {};
        places.push({
          name,
          street:      p.street       || parsed.street    || '',
          suburb:      p.city         || parsed.city       || '',
          postcode:    p.postalCode   || parsed.postalCode || '',
      ...
      const perQuery = Math.min(100, Math.max(20, Math.round(400 / selected.length)));
      const queries  = selected.map(([, searchQuery]) => `${searchQuery} ${suburb} Australia`);
    ...
    // ── HERE Places pass (street mode, if key is set) ──────────
    // Fetches every establishment within 400m of the street centre —
    // not search-ranked, so catches businesses Google Maps misses.
    ...
    if (street && hereKey) {
      try {
        ...
        const newFromHere   = herePlaces.filter(p => !existingNames.has(p.name.toLowerCase()));
        setStatus(`HERE Places: ${newFromHere.length} new businesses not found by Google`);
        places = places.concat(newFromHere);
      } catch (e) {
        console.warn('HERE Places failed:', e.message);
        setStatus(`HERE Places unavailable — proceeding with Google results only`);
        await sleep(1500);
      }
    }

    // ── ABN owner enrichment (if GUID is set) ──────────────────
    // Looks up each business by name in the ABN register.
    // For sole traders the legal entity name becomes the Owner field.
    const abnGuid = getAbnGuid();
    if (abnGuid) await abnEnrichOwners(places, abnGuid);
...
function saveCSV() {
  ...
  allPlaces.forEach(p => {
    ...
    rows.push([
      ...
      csv(p.category),          // Industry (Trinity) — trade category from scrape
...
// ── HERE Places API ────────────────────────────────────────
function getHereKey() { return localStorage.getItem('here_key') || ''; }
function setHereKey(k) { localStorage.setItem('here_key', k); }

function promptHereKey() {
  document.getElementById('here-input').value = getHereKey();
  document.getElementById('here-modal').classList.add('active');
  setTimeout(() => document.getElementById('here-input').focus(), 50);
}
function closeHereModal() { document.getElementById('here-modal').classList.remove('active'); }
function saveHereKey() {
  const v = document.getElementById('here-input').value.trim();
  if (!v) { alert('Please enter your HERE API key.'); return; }
  setHereKey(v); closeHereModal();
  alert('HERE API key saved. Street searches will now use HERE Places for complete coverage.');
}
document.getElementById('here-modal').addEventListener('click', e => {
  if (e.target === document.getElementById('here-modal')) closeHereModal();
});

// Fetch all places near a point using HERE Browse API.
// Returns every establishment within radiusMeters regardless of business type.
async function hereBrowse(lat, lng, radiusMeters, apiKey) {
  const allItems = [];
  let nextUrl = `https://browse.search.hereapi.com/v1/browse?at=${lat},${lng}&limit=100&radius=${radiusMeters}&apiKey=${apiKey}`;
  // Follow pagination (HERE returns up to 100 per page)
  while (nextUrl) {
    const r = await fetch(nextUrl);
    if (!r.ok) throw new Error(`HERE API error ${r.status}: ` + (await r.text()).slice(0, 200));
    const data = await r.json();
    allItems.push(...(data.items || []));
    nextUrl = data.next || null;
    if (nextUrl && !nextUrl.includes('apiKey')) nextUrl += `&apiKey=${apiKey}`;
  }
  return allItems;
}

// Convert HERE place items to our internal place format
function herePlacesToPlaces(items, streetKeyword, suburbLower) {
  const places = [];
  const seen   = new Set();
  for (const item of items) {
    ...
    const streetFl = ((addr.houseNumber || '') + ' ' + (addr.street || '')).trim();
    const fullAddr = addr.label || '';
    const suburb   = addr.city || addr.district || '';
    const addrCheck = (streetFl + ' ' + fullAddr).toLowerCase();

    // Apply same street + suburb filter
    if (!addrCheck.includes(streetKeyword)) continue;
    const suburbCheck = suburb.toLowerCase();
    if (suburbCheck && !suburbCheck.includes(suburbLower) && !suburbLower.includes(suburbCheck)) continue;

    seen.add(name.toLowerCase());
    const contacts = (item.contacts || [])[0] || {};
    const phone    = (contacts.phone || [])[0]?.value || '';
    const website  = (contacts.www   || [])[0]?.value || '';
...
// ── ABN Lookup ─────────────────────────────────────────────
function getAbnGuid() { return localStorage.getItem('abn_guid') || ''; }
function setAbnGuid(g) { localStorage.setItem('abn_guid', g); }

function promptAbnGuid() {
  document.getElementById('abn-input').value = getAbnGuid();
  document.getElementById('abn-modal').classList.add('active');
  setTimeout(() => document.getElementById('abn-input').focus(), 50);
}
function closeAbnModal() { document.getElementById('abn-modal').classList.remove('active'); }
function saveAbnGuid() {
  const v = document.getElementById('abn-input').value.trim();
  if (!v) { alert('Please enter your ABN GUID.'); return; }
  setAbnGuid(v); closeAbnModal();
  alert('ABN GUID saved. Street searches will now include ABN register results.');
}
document.getElementById('abn-modal').addEventListener('click', e => {
  if (e.target === document.getElementById('abn-modal')) closeAbnModal();
});

// ABN API returns JSONP: callback({...}) — strip the wrapper before parsing
async function abnFetch(url) {
  const parse = text => {
    const json = text.trim().replace(/^[^(]+\(/, '').replace(/\)\s*;?\s*$/, '');
    return JSON.parse(json);
  };
  // Try direct first, then CORS proxy
  for (const u of [url, `https://corsproxy.io/?url=${encodeURIComponent(url)}`]) {
    try {
      const r = await fetch(u);
      if (r.ok) return parse(await r.text());
    } catch (e) { /* try next */ }
  }
  throw new Error('ABN fetch failed for: ' + url);
}

// Enrich existing places with owner name from ABN register
async function abnEnrichOwners(places, guid) {
  if (!guid || !places.length) return;
  let enriched = 0;
  for (let i = 0; i < places.length; i++) {
    if (places[i].owner) continue; // already have an owner
    setStatus(`ABN owner lookup ${i + 1}/${places.length}: ${places[i].name}`);
    try {
      const url = `https://abr.business.gov.au/json/MatchingNames.aspx?name=${encodeURIComponent(places[i].name)}&guid=${guid}&activeABNsOnly=true`;
      const data = await abnFetch(url);
      const names = (data.Names || []).filter(n => n.State === 'QLD' || !n.State);
      if (!names.length) continue;
      // Prefer same postcode, else take highest score
      const match = names.find(n => n.Postcode === places[i].postcode) || names[0];
      // Get full details for entity type
      const det = await abnFetch(`https://abr.business.gov.au/json/AbnDetails.aspx?abn=${match.Abn}&guid=${guid}`);
      if (det.EntityTypeCode === 'IND') { // Individual / Sole Trader
        places[i].owner = det.EntityName || match.Name;
        places[i].abn   = match.Abn;
        enriched++;
      } else if (det.EntityName) {
        places[i].abn = match.Abn; // store ABN even for companies
      }
    } catch (e) { /* skip */ }
    await sleep(150);
  }
  setStatus(`ABN enrichment done — ${enriched} owner name${enriched !== 1 ? 's' : ''} found.`);
}
