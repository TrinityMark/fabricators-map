...
async function runSearch() {
  ...
  try {
    ...
    // ── ABN enrichment (street mode only, if GUID is set) ──────
    const abnGuid = getAbnGuid();
    if (street && abnGuid) {
      // Pass 4: find businesses in the ABN register not returned by Google
      const seenNames = new Set(places.map(p => p.name.toLowerCase()));
      const postcode  = places.find(p => p.postcode)?.postcode || '4500';
      const abnPlaces = await abnStreetSearch(street, suburb, postcode, seenNames, abnGuid);

      // Geocode any ABN-only businesses so they can be placed on the map
      for (const p of abnPlaces) {
        try {
          const c = await geocode(`${p.street} ${p.suburb} Australia`);
          p.lat = c.lat; p.lng = c.lng;
        } catch (e) {
          // Use suburb centre if geocode fails
          p.lat = center.lat; p.lng = center.lng;
        }
      }
      places = places.concat(abnPlaces.filter(p => p.lat && p.lng));

      // Enrich owner names for all places
      await abnEnrichOwners(places, abnGuid);
    }
...
// Fetch via CORS proxy if direct fetch fails (ABN API may not have CORS headers)
async function abnFetch(url) {
  ...
  if (!r2.ok) throw new Error('ABN fetch failed: ' + r2.status);
  return r2.json();
}

// Search ABN register by postcode, filter to street keyword, return places not already seen
async function abnStreetSearch(street, suburb, postcode, seenNames, guid) {
  if (!guid) return [];
  const keyword = street.toLowerCase().split(/\s+/)[0];
  const pc = postcode || '4500'; // fallback to Brendale postcode

  setStatus(`ABN register: searching postcode ${pc} for ${street}…`);
  const url = `https://abr.business.gov.au/json/MatchingNames.aspx?name=${encodeURIComponent(street)}&guid=${guid}&postcode=${pc}&activeABNsOnly=true`;

  let names;
  try { names = (await abnFetch(url)).Names || []; }
  catch (e) { console.warn('ABN street search failed:', e); return []; }

  const newPlaces = [];
  for (const n of names) {
    if (seenNames.has(n.Name.toLowerCase())) continue;
    // Get full details to check registered address
    try {
      const det = await abnFetch(`https://abr.business.gov.au/json/AbnDetails.aspx?abn=${n.Abn}&guid=${guid}`);
      const addr = ((det.AddressPostcode || '') + ' ' + (det.AddressState || '') + ' ' + (det.AddressLine1 || '')).toLowerCase();
      if (!addr.includes(keyword)) continue; // not on this street
      const name = det.EntityName || n.Name;
      if (seenNames.has(name.toLowerCase())) continue;
      seenNames.add(name.toLowerCase());
      newPlaces.push({
        name,
        street:      det.AddressLine1 || '',
        suburb:      det.AddressSuburb || suburb,
        postcode:    det.AddressPostcode || pc,
        state:       det.AddressState || 'QLD',
        phone:       '',
        website:     '',
        rating:      '',
        owner:       det.EntityTypeCode === 'IND' ? name : '',
        category:    'ABN Register',
        fullAddress: `${det.AddressLine1 || ''}, ${det.AddressSuburb || suburb} ${det.AddressState || 'QLD'} ${det.AddressPostcode || pc}`,
        abn:         n.Abn,
        lat: null, lng: null, // no coords from ABN — geocode separately
      });
